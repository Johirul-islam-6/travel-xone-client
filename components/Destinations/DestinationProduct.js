...
const DestinationProduct = (props) => {
  ...
  const [cetagoryId, sethotel] = useState('643c2ace24a8114c69217528')
  // '643c2ace24a8114c69217528'


  const DynamicArea = (id) => {
    console.log("age", cetagoryId)
    sethotel(placeId)
    console.log("pore", cetagoryId)
  }

  const id = cetagoryId;
  // ------dynamic area hotel get--------
  useEffect(() => {
    fetch(`https://travel-xone-server.vercel.app/api/v1/hotels?placeID=${id}`)
      .then(res => res.json())
      .then(data => {
        sethotelAll(data.data)
        console.log("cetagory hotel", data)
        setLoding(false)
        DynamicArea()
      })
  }, [sethotelAll]);
  ...
  useEffect(() => {
    fetch(`https://travel-xone-server.vercel.app/api/v1/hotels/`) //?placeID=643c2ace24a8114c69217529
      ...
      .then(data => {
        ...
        // console.log("hoten Name", data?.data)
        setLoding(false)
      })
  }, []);
          ...
          <div className="bg-gray-100">
            <div className="">
              ...
              <div className="flex-wrap md:flex lg:flex gap-5 mt-2">
                <div onClick={() => DynamicArea()} className={`bg-slate-100 text-zinc-500 py-1 px-2 rounded hover:bg-[#627FF4] hover:text-white shadow text-sm my-2 md:my-0 lg:my-0 cursor-pointer`}>All</div>
                {
                  hotehotelCetagory?.map(place => <div onClick={() => DynamicArea(setPlace(place?.placeID))} className={`bg-slate-100 text-zinc-500 py-1 px-2 rounded hover:bg-[#627FF4] hover:text-white shadow text-sm my-2 md:my-0 lg:my-0 cursor-pointer`}>{place?.placeName}</div>)
                }
              </div>
              {/*------- Location Button End------- */}
              {/*------- package body Start------- */}
